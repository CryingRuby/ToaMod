...
public enum ReforgeType {
	FORCEFUL(0, "forceful", ToaItemType.SWORD, (new ToaProperties("Reforge", ToaRarity.UNCOMMON)).str(new float[]{10, 20, 30, 45, 60, 75, 95}).cr(1).cd(1)), 
	STRONG(1, "strong", ToaItemType.SWORD, (new ToaProperties("Reforge", ToaRarity.COMMON)).str(new float[]{25, 40, 55, 75, 100, 135, 200})), 
	WISE(2, "wise", ToaItemType.SWORD, (new ToaProperties("Reforge", ToaRarity.RARE)).wis(new float[]{20, 35, 55, 80, 110, 150, 200})), 
	FLEXIBLE(3, "flexible", ToaItemType.ARMOR, (new ToaProperties("Reforge", ToaRarity.COMMON)).dex(new float[]{10, 20, 30, 40, 50, 70, 90})), 
	BULLETPROOF(4, "bulletproof", ToaItemType.ARMOR, (new ToaProperties("Reforge", ToaRarity.UNCOMMON)).hp(new float[]{15, 30, 45, 60, 75, 95, 125}).ar(new float[]{20, 40, 60, 80, 100, 140, 180})), 
	MYSTIC(5, "mystic", ToaItemType.ARMOR, (new ToaProperties("Reforge", ToaRarity.UNCOMMON)).hp(new float[]{15, 30, 45, 60, 75, 95, 125}).mr(new float[]{20, 40, 60, 80, 100, 140, 180})), 
	MAGICAL(6, "magical", ToaItemType.SWORD, (new ToaProperties("Reforge", ToaRarity.COMMON))._int(new float[]{20, 35, 55, 80, 110, 150, 200})), 
	...
	PRECISE(13, "precise", ToaItemType.SWORD, (new ToaProperties("Reforge", ToaRarity.LEGENDARY)).str(new float[]{0, -5, -10, -20, -30, -45, -60}).cr(new float[]{5, 8, 12, 16, 20, 25, 33}).cd(new float[]{30, 65, 100, 140, 185, 235, 300})), 
	...
	public final ToaItemType itemType;
	public final String name;

	/**
	 * @param itemType are the types of items this reforge can be used for
	 */
	private ReforgeType(int id, String name, ToaItemType itemType, ToaProperties stats) {
		this.name = name;
