...
public class OnFighterRewardsTickProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (emptySlots) {
			...
			for (int index1 = 0; index1 < 20; index1++) {
				if ((entity instanceof Player _plrSlotItem && _plrSlotItem.containerMenu instanceof Supplier _splr && _splr.get() instanceof Map _slt ? ((Slot) _slt.get((int) i)).getItem() : ItemStack.EMPTY).getItem() == Blocks.AIR.asItem()) {
					if (i <= (entity.getCapability(ToamodModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new ToamodModVariables.PlayerVariables())).combatMilestoneLvl) {
						if (i % 5 == 0) {
						...
						if (i - 1 == (entity.getCapability(ToamodModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new ToamodModVariables.PlayerVariables())).combatMilestoneLvl) {
							progressTxt = ",'{\"text\":\" \"}','{\"text\":\"\u00A7a";
							for (int index2 = 0; index2 < (int) Math.floor(((entity.getCapability(ToamodModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new ToamodModVariables.PlayerVariables())).totalMobsKilled
									/ (entity.getCapability(ToamodModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new ToamodModVariables.PlayerVariables())).combatMilestoneKillsNeeded) * 10); index2++) {
								progressTxt = progressTxt + "-";
							}
							progressTxt = progressTxt + "\u00A77";
							for (int index3 = 0; index3 < (int) (10 - Math.floor(((entity.getCapability(ToamodModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new ToamodModVariables.PlayerVariables())).totalMobsKilled
									/ (entity.getCapability(ToamodModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new ToamodModVariables.PlayerVariables())).combatMilestoneKillsNeeded) * 10)); index3++) {
								progressTxt = progressTxt + "-";
							}
							progressTxt = progressTxt + " \u00A7e"
									+ new java.text.DecimalFormat("##").format((entity.getCapability(ToamodModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new ToamodModVariables.PlayerVariables())).totalMobsKilled) + " \u00A77/ \u00A7e"
									...
									+ " \u00A77(\u00A7e"
									+ new java.text.DecimalFormat("##.##").format((entity.getCapability(ToamodModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new ToamodModVariables.PlayerVariables())).combatMilestoneProgress)
									+ "%\u00A77)\u00A7r\"}'";
						} else {
							progressTxt = "";
						}
						...
						if (!_ent.level().isClientSide() && _ent.getServer() != null) {
							_ent.getServer().getCommands().performPrefixedCommand(
									...
									("data modify entity @e[type=toamod:gui_item_ghost,limit=1,distance=..2] HandItems[0] set value {id:\"" + "" + itemId + "\",Count:1,tag:{display:{Name:'{\"text\":\"\u00A7a\u00A7lLevel "
											...
											+ Component.translatable(("milestone.fighter.rewards." + new java.text.DecimalFormat("##").format(i))).getString() + progressTxt + "]}}}"));
						}
					}
					{
						final Vec3 _center = new Vec3(x, y, z);
